...
public class DCT implements PlugInFilter {
			...
			for (int y = 0; y < ip.getHeight(); y += BLOCK_SIZE) {
				...
				for (int i = 0; i < BLOCK_SIZE; ++i) {
					for (int j = 0; j < BLOCK_SIZE; ++j) {
						fp.putPixelValue(x + i, y + j,
								Math.round(fp.getPixelValue(x + i, y + j)));
					}
				}
			}
		}
	...
	public double[] forwardDCT1D(double[] imageOrigine) {
		...
		for (int u = 0; u < tailleSignal; u++) {
			...
			for (int m = 0; m < tailleSignal; m++) {
				somme = somme + imageOrigine[m]
						* Math.cos(Math.PI * (m + 0.5) * u / tailleSignal);
			}
	 ...
	 * 
	 * @param signalTransform
	 *            (u) Signal 1D transformé (double[])
		...
		for (int m = 0; m < tailleSignal; m++) {
			...
			for (int u = 0; u < tailleSignal; u++) {
				...
				if (u == 0)
					cu = 1.0 / Math.sqrt(2); // Facteur de normalisation
												// dépendant de u
				somme = somme + cu * signalTransform[u]
						* Math.cos(Math.PI * (m + 0.5) * u / tailleSignal);
			}
			inverse[m] = k * somme;
		}
