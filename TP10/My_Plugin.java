 ...
 *
 * @author FRANCOIS Remy and MIRANDA Yoan
 *
 */
...
public class My_Plugin implements PlugInFilter, DialogListener {
	...
	public void run(ImageProcessor ip) {
		...
		/* à compléter */
		int tailleMasque=(int)Math.ceil(sigma*6);
		if (tailleMasque%2 ==0) 
			tailleMasque++;
		float[] masqueLog = masqueLoG(tailleMasque,sigma);
		...
		if (seuillageZeroCross) {
			ByteProcessor laplacienZeros = this.laplacienZero(fpLaplacian, seuilZeroCross);
			ImagePlus imagePlus = new ImagePlus(titre+"_exo2"+extension, laplacienZeros);
			imagePlus.show();
		}
	}
	...
	public ByteProcessor laplacienZero(ImageProcessor imLaplacien, Float seuil) {
		...
		int ecart=1;
		for(int i=ecart; i<width-ecart;i++){
			for(int j=ecart;j<height-ecart;j++){
				min=imLaplacien.get(i,j);
				max=min;
				for(int k=-ecart;k<=ecart;k++){
					for(int l=-ecart;l<=ecart;l++){
						if(imLaplacien.getf(i+k,j+l)<min)
							min=imLaplacien.getf(i+k,j+l);
						if(imLaplacien.get(i+k,j+l)>max)
							max=imLaplacien.getf(i+k,j+l);							
					}
				}
